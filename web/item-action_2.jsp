...
try {
    ...
    if (resultSet.next()) {
        ...
        if (checkResult.next() && checkResult.getInt("count") > 0) {
            ...
            String errorMessage1 = "Error: Combination of Partnerid: " + partnerid + " already has Itemid: " + itemId;
            out.println("<script>");
            out.println("Swal.fire({");
            out.println("  icon: 'info',");
            out.println("  title: 'Please Check Once Again',");
            out.println("  text: '" + errorMessage1 + "',");
            ...
            if (nullItemResult.next() && nullItemResult.getInt("count") > 0) {
                ...
                if (existingItemId == null) {
                    ...
                    if (rowsAffected > 0) {
                        ...
                        String successMessage1 = "Updated successfully. Partnerid: " + partnerid + ", Itemid: " + itemId;
                        out.println("<script>");
                        out.println("Swal.fire({");
                        out.println("  icon: 'success',");
                        out.println("  title: 'Success',");
                        out.println("  text: '" + successMessage1 + "',");
                        out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
                        out.println("</script>");
                    } else {
                        // Handle the case where no rows were updated
                        String errorMessage2 = "Update Failed";
                        out.println("<script>");
                        out.println("Swal.fire({");
                        out.println("  icon: 'error',");
                        out.println("  title: 'Error',");
                        out.println("  text: '" + errorMessage2 + "',");
                        out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
                        out.println("</script>");
                    }
                } else {
                    // Handle the case where the record already has a non-null item_id
                    String errorMessage3 = "Error: Combination of Partnerid: " + partnerid + " already has Itemid: " + existingItemId;
                    out.println("<script>");
                    out.println("Swal.fire({");
                    out.println("  icon: 'info',");
                    out.println("  title: 'Please Check Once Again',");
                    out.println("  text: '" + errorMessage3 + "',");
                    out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
                    out.println("</script>");
                }
            } else {
                ...
                if (insertionResult > 0) {
                    ...
                    String successMessage2 = "Inserted successfully. Partnerid: " + partnerid + ", Itemid: " + itemId;
                    out.println("<script>");
                    out.println("Swal.fire({");
                    out.println("  icon: 'success',");
                    out.println("  title: 'Success',");
                    out.println("  text: '" + successMessage2 + "',");
                    out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
                    out.println("</script>");
                } else {
        ...
        String errorMessage4 = "Error: No matching email found in partners table.";
        out.println("<script>");
        out.println("Swal.fire({");
        out.println("  icon: 'error',");
        out.println("  title: 'Error',");
        out.println("  text: '" + errorMessage4 + "',");
        out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
        out.println("</script>");
    }
...
try {
    ...
    if (resultSet.next()) {
        ...
        if ("Accepted".equals(newStatus)) {
            if (checkResult.next() && checkResult.getInt("count") > 0) {
                // Handle the case where the combination already exists (you can choose to update the record here)
                String errorMessage1 = "Error: Combination of Partnerid: " + partnerid + " already has Itemid: " + itemId;
                out.println("<script>");
                out.println("Swal.fire({");
                out.println("  icon: 'info',");
                ...
                // Insert a new record into partneritems
                String insertPartnerItemSql = "INSERT INTO partneritems (partner_id, item_id) VALUES (?, ?)";
                PreparedStatement insertPartnerItemStatement = connection.prepareStatement(insertPartnerItemSql);
                insertPartnerItemStatement.setString(1, partnerid);
                insertPartnerItemStatement.setString(2, itemId);
                ...
                if (insertionResult > 0) {
                    // Handle successful insertion
                    String successMessage1 = "Inserted successfully. Partnerid: " + partnerid + ", Itemid: " + itemId;
                    out.println("<script>");
                    out.println("Swal.fire({");
                    out.println("  icon: 'success',");
                    out.println("  title: 'Success',");
                    ...
                    // Handle the case where the insertion failed
                    String errorMessage2 = "Error: Failed to insert record. Partnerid: " + partnerid + ", Itemid: " + itemId;
                    out.println("<script>");
                    out.println("Swal.fire({");
                    out.println("  icon: 'error',");
                    out.println("  title: 'Error',");
            ...
            // If newStatus is "Pending," remove the partner item ID

            String removePartnerItemSql = "DELETE FROM partneritems WHERE partner_id = ? AND item_id = ?";
            PreparedStatement removePartnerItemStatement = connection.prepareStatement(removePartnerItemSql);
            removePartnerItemStatement.setString(1, partnerid);
            removePartnerItemStatement.setString(2, itemId);
            int removalResult = removePartnerItemStatement.executeUpdate();
            removePartnerItemStatement.close();

            if (removalResult > 0) {
                // Handle successful removal
                String successMessage3 = "Removed successfully. Partnerid: " + partnerid + ", Itemid: " + itemId;
                out.println("<script>");
                out.println("Swal.fire({");
                out.println("  icon: 'success',");
                out.println("  title: 'Success',");
                out.println("  text: '" + successMessage3 + "',");
                out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
                out.println("</script>");
            } else {
                // Handle the case where the removal failed
                String errorMessage4 = "Error: Failed to remove record. Partnerid: " + partnerid + ", Itemid: " + itemId;
                out.println("<script>");
                out.println("Swal.fire({");
                out.println("  icon: 'error',");
                out.println("  title: 'Error',");
        ...
        // Handle the case where no matching email is found in the partners table
        String errorMessage5 = "Error: No matching email found in partners table.";
        out.println("<script>");
        out.println("Swal.fire({");
        out.println("  icon: 'error',");
        out.println("  title: 'Error',");
        out.println("  text: '" + errorMessage5 + "',");
        out.println("}).then(function() { window.location.href = 'partner_all_bookings.jsp'; });");
        out.println("</script>");
    }
