<%-- 
    ...
    Created on : 02-Sept-2023, 4:15:53 pm
    Author     : manikanta
--%>
...
try {
    ...
    if (ServletFileUpload.isMultipartContent(request)) {
        ...
        try (Connection connection = new Dbconn.dbconn().getConnection();
             PreparedStatement checkEmailStatement = connection.prepareStatement("SELECT email FROM customers WHERE email = ?");
        ) {
        ...
        if (!emailExists) {
            ...
            String sql = "INSERT INTO customers(firstname, email,phone,pincode,address, state, password, image, imagename) VALUES (?,?,?, ?, ?, ?, ?, ?, ?)";
            try (Connection connection = new Dbconn.dbconn().getConnection();
            ...
            statement.setBinaryStream(8, input);
            statement.setString(9, fileName);
                ...
                // response.sendRedirect("customer_dashboard.jsp");
                   ...
                   response.sendRedirect("customer_register.jsp?email=" + session.getAttribute("email"));
                }
            } catch (Exception e) {
                out.print(e);
            }
            ...
            response.sendRedirect("candidate-login.jsp?email=" + session.getAttribute("email"));
        }
    }
} catch (Exception e) {
    out.print(e);
