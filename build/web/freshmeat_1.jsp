...
<html lang="zxx">
    ...
    <head>
        ...
        <title>D-LICIOUS | Freshmeat</title>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-md-4 col-sm-6 mix vegetables">
                                <div class="featured__item">
                                    <form id="priceForm" action="ordering-action2.jsp?email=<%=email%>&itemid=<%=itemid%>" method="post"  >    <!--enctype="multipart/form-data -->
                                        <div class="flip-card">
                                                    ...
                                                    <table>
                                                        ...
                                                        <div>
                                                            <ul class="featured__item__pic__hover2 featured__item__pic2 ">
                                                                ...
                                                                <li><a href="overview.jsp?itemid=<%=itemid%>&category=<%=category1%>&email=${sessionScope.email}"><i class="fa bi bi-info"></i></a></li>
                                                        ...
                                                        <tr>
                                                            <td style="background-color: #a3c2c2;color: white;">Quantity:</td>
                                                            ...
                                                            <td style="background-color: #badecc;color: black;"><h6><%= price%></h6></td>                                                        </tr>
                                                    </table>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="featured__item__text">
                                            <div class="card" data-price-per-kg="<%= price%>" data-discount-percentage="<%= discount%>" data-item-name="<%= itemName%>" data-price="<%= price%>" data-image="<%= base64Image%>"
                                                  data-category="<%= category1%>">
                                                <div class="input-group">
                                                    <span class="input-group-text">Enter Quantity:</span>
                                                    <input type="text" name="userquantity" class="form-control quantity-input" placeholder="in Kgs">
                                                </div>
                                                <div class="input-group">
...
<!--                                                    <input type="hidden" name="image" value="<%=base64Image%>">
                                                    ...
                                                    <input type="text" name="generatedprice" class="form-control price-input" value="" readonly>
                                                    <input type="hidden" id="generatedPrice" name="generatedPrice" value="">-->
                                                     <input type="hidden" name="image" class="image-input" value="<%=base64Image%>">
                        <input type="hidden" name="category" class="category-input" value="<%=category1%>">
                        <input type="hidden" name="itemname" class="item-name-input" value="<%=itemName%>">
                        <input type="text" name="generatedprice" class="form-control price-input" value="" readonly>
                        <input type="hidden" id="generatedPrice" name="generatedPrice" value="">
                                                </div>
                                            </div>
                      ...
                      <input type="button" class="btn btn-primary mt-2" value="Submit" onclick="submitForm()">
                                        </div>
                                    </form>
                                </div>
                            </div>
                            <script>
                                document.addEventListener('DOMContentLoaded', function () {
                                    ...
                                    cards.forEach((card) => {
                                        // Get references to quantity and price input fields within the card
                                        const quantityInput = card.querySelector('.quantity-input');
                                        const priceInput = card.querySelector('.price-input');
                              
    

                                        // Add an input event listener to calculate price based on user input
                                        quantityInput.addEventListener('input', function () {
                                            const quantityInKg = parseFloat(this.value);

                                            if (!isNaN(quantityInKg)) {
                                                const pricePerKg = parseFloat(card.getAttribute('data-price-per-kg'));
                                                const discountPercentage = parseFloat(card.getAttribute('data-discount-percentage'));
                                                const totalPrice = quantityInKg * pricePerKg;
                                                const discountAmount = (totalPrice * discountPercentage) / 100;
                                                const discountedPrice = totalPrice - discountAmount;
                                                priceInput.value = discountedPrice.toFixed(2);
                                               
                                               // document.getElementById('generatedPrice').value = discountedPrice.toFixed(2);
...
//                             
function submitForm() {
    // Access the generated price input field
    const generatedPriceInput = document.getElementById('generatedPrice');
    
    // Calculate the generated price based on the current quantity input
    const quantityInput = document.querySelector('.quantity-input');
    const quantityInKg = parseFloat(quantityInput.value);

    if (!isNaN(quantityInKg)) {
        const pricePerKg = parseFloat(quantityInput.closest('.card').getAttribute('data-price-per-kg'));
        const discountPercentage = parseFloat(quantityInput.closest('.card').getAttribute('data-discount-percentage'));

        const totalPrice = quantityInKg * pricePerKg;
        const discountAmount = (totalPrice * discountPercentage) / 100;
        const discountedPrice = totalPrice - discountAmount;

        // Set the generated price in the hidden input field
        generatedPriceInput.value = discountedPrice.toFixed(2);
    } else {
        // Handle the case when quantity is not a valid number
        generatedPriceInput.value = '';
    }
